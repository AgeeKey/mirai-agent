...
class RiskEngine:
    ...
    def allow_entry(self, now_utc: datetime, symbol: str, account_state: Dict[str, Any] = None) -> Tuple[bool, str]:
        ...
        if day_state.day_pnl <= self.config['DAILY_MAX_LOSS']:
            return False, f"Daily max loss reached: {day_state.day_pnl} <= {self.config['DAILY_MAX_LOSS']}"
        
        # Gate b) Daily trail drawdown
        if day_state.max_day_pnl - day_state.day_pnl >= self.config['DAILY_TRAIL_DRAWDOWN']:
            return False, f"Daily trail drawdown exceeded: {day_state.max_day_pnl - day_state.day_pnl} >= {self.config['DAILY_TRAIL_DRAWDOWN']}"
        
        # Gate c) Trade limit
        if day_state.trades_today >= self.config['MAX_TRADES_PER_DAY']:
            return False, f"Max trades per day reached: {day_state.trades_today} >= {self.config['MAX_TRADES_PER_DAY']}"
        ...
        if day_state.consecutive_losses >= self.config['MAX_CONSECUTIVE_LOSSES']:
            if day_state.cooldown_until:
                ...
                if now_utc < cooldown_time:
                    return False, f"In cooldown until {day_state.cooldown_until} after {day_state.consecutive_losses} consecutive losses"
